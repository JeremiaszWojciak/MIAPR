...
bool isStateValid(const ob::State *state){
    ...
    auto pos_x = coordX->values[0] - occupancyMap.info.origin.position.x;
    auto pos_y = coordY->values[0] - occupancyMap.info.origin.position.y;

    auto pos_x_map = static_cast<uint32_t>(pos_x / occupancyMap.info.resolution);
    auto pos_y_map = static_cast<uint32_t>(pos_y / occupancyMap.info.resolution);

    // if(occupancyMap.data[pos_x_map + pos_y_map * occupancyMap.info.width] != 0) {
    //     return false;
    // }

    // if(occupancyMap.data[pos_x_map + pos_y_map * occupancyMap.info.width] == 100) {
    //     return false;
    // }

    for(int i=-4; i<5; i++) {
        for(int j=-4; j<5; j++) {
            if(occupancyMap.data[pos_x_map + i + (pos_y_map + j) * occupancyMap.info.width] == 100) {
                return false;
            }
        }
    }
